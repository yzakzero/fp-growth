...
if __name__=="__main__":
	...
	# min_support=500#最小支持度（看起来是个数）
	# min_conf=0.9#最小置信度（百分比）

	l1 = []

	# 针对标题
	words = readtitle('dataset/dataL.csv')

	# 针对标题与摘要共同处理
	#words = read_ab_and_ti('dataset/dataL.csv')

	w = ['A', 'The', 'An', 'There', 'We', 'This', 'In', 'It', 'I', 'Most', 'Open']
	...
	for iwords in words:
		filtered_words = [word for word in iwords if word not in s]
		#print('原始词：', iwords)
	...
	data_set=words
	filename="dataL.csv"
	min_support=125#最小支持度
	min_conf=0.7#最小置信度

	spend_time=[]
	current_path=os.getcwd()

	save_path=current_path+"/log/"+filename+"_fpgrowth.txt"
